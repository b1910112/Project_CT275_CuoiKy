...
if(isset($_GET['search'])) {
    $search = $_GET['search'];
    $sql = "SELECT * FROM `story` WHERE `categogy` = 'Hài Hước' AND (`storyname` LIKE '%$search%' OR `author` LIKE '%$search%') ORDER BY `id` DESC LIMIT $start_from, $results_per_page";
} else {
    $sql = "SELECT * FROM `story` WHERE `categogy` = 'Hài Hước' ORDER BY `id` DESC LIMIT $start_from, $results_per_page";
}
...
<br>
<div class="container">
    <h1 class="text-center">Truyện Hài Hước</h1>
    <br>
    <div class="row">
